...
- **Слэш-команды** — управляйте доской прямо из чата Qwen Code: `/todo`, `/todo-next`, `/todo-next-run`, `/todo-list`
...
│                     │  │                             │
│ Запуск:             │  │ Проект B:                   │
│   python -m server  │  │   .qwen/todo-ai.json        │
│   .main             │  │   → board: "project-B"      │
│                     │  │   → работает с project-B    │
└─────────────────────┘  └─────────────────────────────┘
```

### Как это работает

1. **UI** — запускается один раз, показывает все доски, переключается между ними через UI
...
| `/todo-next-run` | Взять следующую задачу из "Сделать" и переместить в "В работе" (status 2 -> 3) |
...
- Запустите сервер: `SERVER_MODE=http BOARD_NAME=main APP_PORT=8167 python -m server.main`
- Проверьте что порт свободен: `curl http://localhost:8167/health`
